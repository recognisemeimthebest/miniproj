...
293/64/63 label-stratified 70/15/15 partition originally produced by the
3D-CNN pipeline (stored at results/ct/features/*.npz). Local copies live
inside this single_modal_baseline folder so teammates can run the scripts
...
SPLIT_FEATURES = FOLDER_ROOT / "results" / "ct" / "features"
...
def load_split_pids() -> dict[str, list[str]]:
    ...
    for split in ("train", "val", "test"):
        d = np.load(SPLIT_FEATURES / f"{split}.npz", allow_pickle=True)
