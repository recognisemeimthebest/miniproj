...
Pipeline
--------
1. Load preprocessing/output_clinical/lung1_clinical_encoded.csv
...
4. Final retrain with the best params:
     ...
     - trainval   : train on 293+64=357, report test AUROC (headline number)
5. Save results + best params + model weights under
     experiments/single_modal/clinical/

The MLP architecture mirrors the CSJ ClinicalBranch design
(Linear -> BN -> ReLU -> Dropout) x2 then an embedding head and a tiny
classifier that is used here only to drive training.
"""
...
OUT_DIR = REPO_ROOT / "experiments" / "single_modal" / "clinical"
...
def load_clinical_frame() -> tuple[pd.DataFrame, list[str]]:
    df = pd.read_csv(CLINICAL_CSV, index_col="PatientID")
...
class ClinicalBranch(nn.Module):
    """MLP encoder + binary classifier head (CSJ design)."""
...
def _fit(
    ...
    *,
    ...
    """Train ClinicalBranch with given hyperparameters. Optionally track eval AUROC per epoch."""
    set_seed(SEED)
    ...
    loader = DataLoader(
        ClinicalDS(X_tr, y_tr), batch_size=params["batch_size"], shuffle=True
    )
...
# Optuna
# ─────────────────────────────────────────────
def run_optuna(arrays, feat_cols, n_trials: int) -> "optuna.Study":
    import optuna
...
# ─────────────────────────────────────────────
# Final retrain + test evaluation
# ─────────────────────────────────────────────
def _full_metrics(y_true: np.ndarray, prob: np.ndarray, threshold: float = 0.5) -> dict:
    pred = (prob >= threshold).astype(int)
    auroc = float(roc_auc_score(y_true, prob))
    acc = float(accuracy_score(y_true, pred))
    cm = confusion_matrix(y_true, pred).tolist()
    p0, r0, f0, _ = precision_recall_fscore_support(
        y_true, pred, labels=[0], zero_division=0
    )
...
def final_retrain_and_eval(arrays, feat_cols, best_params: dict) -> dict:
    X_tr_raw, y_tr, _ = arrays["train"]
    X_va_raw, y_va, _ = arrays["val"]
    ...
    # No val during fit — simply train fixed epochs with best_params
    model_tv, _ = _fit(
    ...
    # ── Save test predictions (trainval) ──
...
def main(n_trials: int) -> None:
    set_seed(SEED)
    print(f"[clinical] device={DEVICE} csv={CLINICAL_CSV}")
    df, feat_cols = load_clinical_frame()
    print(f"[clinical] n_all={len(df)} feat_cols={len(feat_cols)} → {feat_cols}")

    arrays = build_split_arrays(df, feat_cols)
    for k, (X, y, _) in arrays.items():
        print(f"  {k}: X={X.shape}  pos={int(y.sum())}  neg={int((y==0).sum())}")

    print(f"\n[clinical] Optuna: {n_trials} trials")
    study = run_optuna(arrays, feat_cols, n_trials=n_trials)
    best_params = study.best_params
    best_val_auroc = float(study.best_value)
    print(f"[clinical] best val AUROC (Optuna): {best_val_auroc:.4f}")
    print(f"[clinical] best params: {best_params}")

    print("\n[clinical] Retrain + test evaluation...")
